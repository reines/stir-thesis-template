...
my $printThesisState=0; # Flag to print out only thesis outline with state
...
my %TeXheader=('\title'=>[2],'\part'=>[2],'\chapter'=>[2],'\chapter*'=>[2],
...
my %TeXmacroword=('\LaTeX'=>1,'\TeX'=>1, '\when'=>1, '\whenNS'=>1, '\event'=>1, '\eventNS'=>1, '\trigger'=>1, '\condition'=>1, '\conditionNS'=>1, '\action'=>1, '\and'=>1, '\oror'=>1, '\then'=>1, '\adhoc'=>2, '\etal'=>2, '\eg'=>1, '\url'=>1, '\lps'=>3, '\elseelse'=>1, '\notnot'=>1, '\notnots'=>1, '\ifif'=>1, '\action'=>1, '\dodo'=>1, '\doNS'=>1, '\hec'=>3);
...
my %TeXgroup=('document'=>1,'letter'=>1,'titlepage'=>0,
     ...
     'description'=>1,'enumerate'=>1,'itemize'=>1,'description'=>1,'list'=>1,
...
sub MAIN {
 ...
 push @args,@_;
  ...
  if (scalar @toplevelfiles>0 || $fileFromSTDIN) {
	...
	if ($optionWordFreq || $optionWordClassFreq || $freqSummary) {
		print_word_freq();
	}
	elsif (!$printThesisState) {
...
sub parse_options_parsing {
  ...
  elsif ($arg eq '-freqSummary') {$freqSummary=1;}
  elsif ($arg eq '-parseCommentedIncludes') {$parseCommentedIncludes=1;}
...
sub parse_options_output {
  ...
  elsif ($arg eq '-printThesisState') {$printThesisState=1;}
...
sub tc_macro_param_option {
  ...
  {
	$TeXheader{$macro}=$param;
	$TeXmacro{$macro}=$param;
  }
...
sub _TeXcode_blank {
  my ($filename,$title)=@_;
  if (defined $title) {}
  elsif (defined $filename) { if (!$freqSummary)   {$title="File: ".$filename;} }
  ...
  while (!$tex->{'eof'}) {
    ...
    if ($ch eq '') {
      ...
      return __get_chtoken($tex,$ch,$TOKEN_SYMBOL);
		...
		my $subs;
		$subs = substr($tex->{'line'}, 2, 7);
			
		if ($parseCommentedIncludes && ($subs eq 'include' || $subs eq 'insert{'))
		{
			my $lineminuscomment;
			$lineminuscomment = substr($tex->{'line'}, 1, length($tex->{'line'}));
		
			$tex->{'line'} = $lineminuscomment;
			return __set_token($tex,$1,$TOKEN_MACRO);
		}
		else
		{
...
sub print_word_freq {
  ...
  if ($htmlstyle) {print "<table class='stat'>\n<thead>\n";}
   if (!$freqSummary) {__print_word_freq("Word","Freq","th");}
  ...
  if ($htmlstyle) {print "<tbody class='sumstat'>\n";}
	if ($freqSummary)
	{
		__print_word_freq("",$count,,'td','sum');
	}
	else
	{
		__print_word_freq("Number of unique words",$count,,'td','sum');
	}
...
sub __print_word_freq {
  ...
  if ($htmlstyle) {
    print "<tr",$class,"><",$tag,">",lc($word),"</",$tag,"><",$tag,">",$freq,"</",$tag,"></tr>\n";
  } else {
